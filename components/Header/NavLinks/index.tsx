...
import { IconCalendarEvent, IconFileFilled, IconForms, IconSquareCheckFilled, IconUserFilled } from "@tabler/icons-react";
	...
	return (
		<Group justify="end" gap="md">
			{navButtons.map((navButton) => (
				<Menu shadow="md" width={200} trigger="hover" openDelay={0} closeDelay={0} transitionProps={{ transition: 'slide-left', duration: 150 }}>
					<Menu.Target>
						<Button variant="light">{navButton.text}</Button>
					</Menu.Target>

					<Menu.Dropdown>
						{(navButton.links).map((link) => (
							<Menu.Item component={Link} href={link.href} leftSection={link.icon}>
								{link.text}
							</Menu.Item>
						))}
					</Menu.Dropdown>
